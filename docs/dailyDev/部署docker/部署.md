...
exit
```
<font color="red">如果使用虚拟机的时候需要关闭防火墙，navicat才能进行链接（这是偷懒的方式，最好指定端口进行放开）</font>
1:查看防火状态
...
（项目的中间件使用docker进行部署，文件内容可以参考项目中`docker`文件夹)
...
auth austin   // 在配置文件中配置的权限

```

<font color="red">配置在同一个文件中，在service：的下一级(分开每个文件夹也ok，但是要每个文件夹内部执行启动)</font>
...
<font color="red">当运行docker-compose  up -d结束之后，会产生一个sql文件夹，然后我们将这两个文件放到文件夹中，然后进行docker-compose  up -d 再执行</font>
文件在这
[apollo配置sql1](file/apolloconfigdb.sql)
[apollo配置sql2](file/apolloportaldb.sql)
